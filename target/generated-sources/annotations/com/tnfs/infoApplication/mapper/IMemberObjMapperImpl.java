...
public class IMemberObjMapperImpl implements IMemberObjMapper {
    ...
    public Set<MemberDto> toDto(Set<MemberObj> set) {
        ...
        Set<MemberDto> set1 = new HashSet<MemberDto>( Math.max( (int) ( set.size() / .75f ) + 1, 16 ) );
        for ( MemberObj memberObj : set ) {
            set1.add( toDto( memberObj ) );
        }
    ...
    public Set<MemberObj> toObj(Set<MemberDto> dtoSet) {
        ...
        Set<MemberObj> set = new HashSet<MemberObj>( Math.max( (int) ( dtoSet.size() / .75f ) + 1, 16 ) );
        for ( MemberDto memberDto : dtoSet ) {
            set.add( toObj( memberDto ) );
        }
